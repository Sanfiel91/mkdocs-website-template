/* ═══════════════════════════════════════════════════════
   Analytics: consent, page tracking, conversion events
   ═══════════════════════════════════════════════════════ */
(function () {
  try {
    ...
    if (typeof document$ !== "undefined") {
      document$.subscribe(function () { bootAnalytics(); });
    }
...
/* ═══════════════════════════════════════════════════════
   Design enhancements: scroll reveals + metric counters
   Isolated from analytics so one cannot crash the other.
   ═══════════════════════════════════════════════════════ */
(function () {
  try {
    ...
    function initReveal() {
      ...
      // Signal to CSS that JS can manage the reveal hiding
      ...
      // Safety fallback: force-show everything after 4 seconds
      // in case IntersectionObserver fails on some devices
      setTimeout(function () {
        document.querySelectorAll(".reveal:not(.is-visible)").forEach(function (el) {
          el.classList.add("is-visible");
        });
        document.querySelectorAll(".reveal-stagger:not(.is-visible)").forEach(function (el) {
          el.classList.add("is-visible");
        });
      }, 4000);
    }
    ...
    if (typeof document$ !== "undefined") {
      document$.subscribe(function () { initDesign(); });
    }
  } catch (e) {
    // If design enhancements fail, content stays visible
    // because CSS only hides when .js-reveal-ready is present
  }
})();
